...
def write_proj(fov_deg: float, ratio: float, near: float, far: float, path: str) -> None:
    ...
    m = projection(fov_deg, ratio, near, far)
    row_count = m.size()[0]
...
def main():
    ...
    for degrees in (100., 70., 40.):
        m = projection(degrees, 1.0, 0.1, 100.0)
        print(f"-- fov={degrees} degrees --")
        m.print_matrix()

    print("-- ratio distortion --")
    projection(70., 16 / 9, 0.1, 100.0).print_matrix()
